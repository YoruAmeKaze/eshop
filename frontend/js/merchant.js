...
let _editProductId = null; // 当前编辑的商品 id，null 表示新增
...
async function loadMerchantShop() {
  ...
  document.getElementById('btn-product-manage').onclick = function () {
    document.querySelectorAll('.m-tab').forEach(t => t.style.display = 'none');
    const target = document.getElementById('m-products');
    target.style.display = 'flex';
    target.style.flexDirection = 'column';
    ...
    if (res && res.status === 'success' && Array.isArray(res.data)) {
      ...
      container.innerHTML = `
        <div class="empty-state">
          ...
          <div class="empty-sub">点击右上角添加第一个商品</div>
        </div>`;
    }
  } catch (e) {
...
async function loadMerchantChats() {
  ...
  if (data && data.status === 'success' && Array.isArray(data.data)) {
    ...
    container.innerHTML = `
      <div class="empty-state">
        <div class="empty-icon">💬</div>
        <div class="empty-title">暂无顾客消息</div>
        <div class="empty-sub">聊天接口尚未接入</div>
      </div>`;
  }
}
...
function renderMerchantChatList(container, list) {
  ...
  container.innerHTML = `
    <div class="card" style="padding:4px 16px;">
      ${list.map(c => `
        <div class="chat-item" onclick="openMerchantChat(${c.id})">
          <div class="chat-avatar">👤</div>
          <div>
            <div class="chat-name">${c.name}</div>
            <div class="chat-preview">${c.last_message || '暂无消息'}</div>
          </div>
        </div>
      `).join('')}
    </div>`;
...
function openMerchantChat(id) {
  window.showToast('聊天功能开发中');
}
