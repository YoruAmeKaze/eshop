    ...
    if (data && data.status === 'success' && Array.isArray(data.data)) {
      ...
      container.innerHTML = `
        <div class="empty-state">
          ...
          <div class="empty-sub">订单接口尚未接入</div>
        </div>`;
    }
  } catch (e) {
  ...
  if (list.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-sub">去首页逛逛吧</div>
      </div>`;
    return;
  }
...
function renderCart(container, list) {
  if (list.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-sub">去首页挑选喜欢的商品吧</div>
      </div>`;
    return;
  }
  ...
  const items = list.map(item => {
    ...
    return `
      <div class="cart-item">
        <div class="cart-img">
          ${item.image_url ? `<img src="${item.image_url}">` : '🛍️'}
        </div>
        <div class="cart-info">
...
window.removeCartItem = async function (goodsId) {
  try {
    ...
    if (data && data.status === 'success') {
      ...
      window.showToast(data?.message || '删除失败');
    }
  } catch (err) {
...
/* ---- 初始化 ---- */
window.addEventListener('consumer:init', () => {
  renderCategoryBar();
  loadProducts();
});
